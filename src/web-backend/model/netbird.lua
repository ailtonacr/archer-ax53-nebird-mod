...
-- Pure helpers: settings persistence (key=value, 0600) + daemon control.
-- No secrets are ever returned; the setup key is handled only by the
-- controller and passed via a 0600 file to the CLI (never argv/logs).
...
-- POSIX-safe single-quote escaping for shell arguments (TP-Link's luci.util
-- has no shellquote; this is self-contained and injects nothing).
local function shellquote(value)
    local s = tostring(value or "")
    return "'" .. s:gsub("'", "'\\''") .. "'"
end

-- paths (must match /lib/netbird/netbird.sh)
...
local function valid_name(v)
    ...
    return #v <= 64 and v:match("^[%w%.%-_]*$") ~= nil
end
...
local function write_settings(cur)
    ...
    nixio.fs.chmod("/tp_data/netbird", 448) -- 0700
...
-- run a control command; args are individually shell-quoted (no injection).
local function run(...)
    local parts = { shellquote(CTL) }
    for i = 1, select("#", ...) do
        parts[#parts + 1] = shellquote(tostring(select(i, ...)))
    end
    return sys.exec(table.concat(parts, " "))
end

-- like run() but also returns the process exit code (for enroll/start/stop)
...
function payload_version()
    return (run("payload-version") or ""):gsub("%s+$", "")
end

function payload_ok()
    local _, rc = run_ex("payload-status")
    return rc == 0
end

function payload_state()
    local out = run("payload-status")
    return (out or ""):match("^%s*(%S+)") or "UNKNOWN"
end
