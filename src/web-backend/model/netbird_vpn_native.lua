...
local function settings_from_config(cfg, profile_key)
    ...
    local advertise_lan = bool01(cfg.advertise_lan, current.advertise_lan or "0")
    local disable_client_routes = bool01(cfg.disable_client_routes, current.disable_client_routes or "1")
    local disable_server_routes = bool01(cfg.disable_server_routes, current.disable_server_routes or "1")
    local disable_firewall = bool01(cfg.disable_firewall, current.disable_firewall or "1")

    -- On this router, LAN routing is intentionally bidirectional: the AX53 is
    -- both a routing peer for the home LAN and a gateway consuming remote
    -- NetBird Networks. Normalize the prerequisites when the mode is enabled.
    ...
    return {
        ...
        -- TP-Link already owns port 53 on the AX53; NetBird DNS must not bind it.
        disable_dns = "1",
        disable_firewall = disable_firewall,
        disable_client_routes = disable_client_routes,
        disable_server_routes = disable_server_routes,
