...
MODEL = [
    # The stock server update path cannot serialize our synthetic NetBird type.
    # Route list-level enable/disable through the dedicated NetBird controller.
    ('async function W(e,n){await function(e,n,t){return a.update(y,{key:e},n,t,{preventSuccess:!0})}(e.key,R(e),R(n))}',
     'async function W(e,n){if(e.type===u.Netbird||n.type===u.Netbird){await nbControl(n.enable?"start":"stop");return}await function(e,n,t){return a.update(y,{key:e},n,t,{preventSuccess:!0})}(e.key,R(e),R(n))}', 1),
    # Deleting the synthetic list row must not hit /admin/vpn?form=server.
    ('async function J(e,n){await function(e,n){return a.remove(y,{key:e,index:n},{preventSuccess:!0})}(e,n)}',
     'async function J(e,n){if(e==="netbird"){await nbControl("clean");return}await function(e,n){return a.remove(y,{key:e,index:n},{preventSuccess:!0})}(e,n)}', 1),
    ('export{_ as A',
...
# Keep the list row aligned with stock semantics: the Server column is the
# configured endpoint, not the overlay address, and speed fields are numeric.
# The endpoint host is displayed without the scheme for parity with WireGuard.
NETBIRD_LIST_NEW = 'i=async()=>{const{data:e,maxRules:t}=await J();let _nb=[];try{const b=await Nbt();if(b&&b.settings&&(b.settings.enrolled==="1"||b.code==="connected"||b.code==="connecting")){let _server=b.settings.management_url||"";try{_server=new URL(_server).host}catch(e){_server=_server.replace(/^https?:\\/\\//,"").replace(/\\/.*$/,"")}const _tr=b.traffic||{};_nb=[{key:"netbird",description:"NetBird",type:it.Netbird,vendor:te.Manual,enable:b.settings.enable==="1",status:"connected"===b.code?we.Connected:"connecting"===b.code?we.Connecting:we.Disconnected,server:_server,uploadSpeed:Number(_tr.uploadSpeed)||0,downloadSpeed:Number(_tr.downloadSpeed)||0}]}}catch(e){}a.value=_nb.concat(e),l.value=t}'
NETBIRD_LIST_OLD = 'i=async()=>{const{data:e,maxRules:t}=await J();let _nb=[];try{const b=await Nbt();if(b&&b.settings&&(b.settings.enrolled==="1"||b.code==="connected"||b.code==="connecting"))_nb=[{key:"netbird",description:"NetBird",type:it.Netbird,vendor:te.Manual,enable:b.settings.enable==="1",status:"connected"===b.code?we.Connected:"connecting"===b.code?we.Connecting:we.Disconnected,server:b.netbird.netbirdIp||b.settings.management_url||"",uploadSpeed:"",downloadSpeed:""}]}catch(e){}a.value=_nb.concat(e),l.value=t}'
...
VPNPAGE = [
    ...
    ('i=async()=>{const{data:e,maxRules:t}=await J();a.value=e,l.value=t}', NETBIRD_LIST_NEW, 1),
...
def patch_text(text, patches, name):
    for old, new, count in patches:
        ...
        legacy_candidates = []
        if new == NETBIRD_LIST_NEW:
            legacy_candidates.extend([NETBIRD_LIST_OLD,
                NETBIRD_LIST_OLD.replace('b.settings&&(b.settings.enrolled==="1"||b.code==="connected"||b.code==="connecting")', 'b.settings&&b.settings.enrolled==="1"'),
                NETBIRD_LIST_OLD.replace('b.settings&&(b.settings.enrolled==="1"||b.code==="connected"||b.code==="connecting")', 'b.settings')])
...
def apply_file(root, rel, is_gzip, patches):
    path = os.path.join(root, rel)
    if not os.path.exists(path):
        raise RuntimeError(f"missing {path}")
    data = open(path, "rb").read()
    text = gzip.decompress(data).decode("utf-8") if is_gzip else data.decode("utf-8")
    text = patch_text(text, patches, rel)
    check_js(rel, text)
    open(path, "wb").write(gzip_roundtrip(text.encode("utf-8")) if is_gzip else text.encode("utf-8"))
