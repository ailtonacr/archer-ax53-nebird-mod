...
dnsproxy_start_deamon()
{
    ...
    if [ -f "/etc/config/dns_privacy" ]; then
        local dns_privacy=`uci get dns_privacy.global.dns_privacy`
        if [ ${dns_privacy} = 'none' ];then
            local anet=$(dnsproxy_anet)
            case $anet in
                4) /usr/lib/dnsproxy/dnsproxy_deamon.sh > /dev/null 2>&1 & ;;
                6) echo "Not support ipv6 now" ;;
                *) echo "Unknown ANET" ;;
            esac
        fi
    else
        echo "does not support dohdot"
        local anet=$(dnsproxy_anet)
            case $anet in
                4) /usr/lib/dnsproxy/dnsproxy_deamon.sh > /dev/null 2>&1 & ;;
                6) echo "Not support ipv6 now" ;;
