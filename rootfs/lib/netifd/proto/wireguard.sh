...
proto_wireguard_setup() 
{
    ...
    local server
    config_load network
    config_get parent "$vpn_interface" "parent"
    config_get server "$vpn_interface" "endpoint_address" && vpnc_check_parent "$parent" && {
        for ip in $(resolveip -4 -t 10 "$server"); do
            ( proto_add_host_dependency "$vpn_interface" "$ip" "$parent" )
            server_addr=1
        done
    }

    [ -n "$server_addr" ] || {
        echo "WireGuard could not resolve server address" > /dev/console
        sleep 5
        proto_setup_failed "$vpn_interface"
        exit 1
    }
