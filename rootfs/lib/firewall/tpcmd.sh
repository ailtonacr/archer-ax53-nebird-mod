...
fw_set_ovpnmss(){
    ...
    case $curr_nw in
        MOBILE)
            basic_mtu=$(uci get modem.modemconf.mtu)
            ;;
        *)
            case $wanproto in
                static|dhcp)
                    basic_mtu=$(uci get network.wan.mtu)
                    ;;
                dslite)
                ...
                v6plus)
                    ...
                    ;;
                *)
                    basic_mtu=$(uci get network.internet.mru)
                    ;;
            esac
            ;;
    esac
    ...
    if [ -n "$basic_mtu" ]; then
        if [ "$wanproto" == "dslite" -o "$wanproto" == "v6plus" ]; then
            size=$((basic_mtu-150))
        else
            size=$((basic_mtu-110))
        fi
    ...
    fw_s_add 4 m $chain "TCPMSS --set-mss $size" { "-i $dev -p tcp -m tcp --tcp-flags SYN,RST SYN" }
    fw_s_add 4 m $chain "TCPMSS --set-mss $size" { "-o $dev -p tcp -m tcp --tcp-flags SYN,RST SYN" }
}
...
fw_openvpn_access(){
    ...
    if [ "$access" == "home" ]; then
        ...
        fw_s_add 4 f forwarding_rule_vpn ACCEPT { "-i $server_dev -o $server_dev" }
    else
    ...
    [ -e /proc/fcache/ ] && {
		local skip_rule=$(fw list 4 m FORWARD | grep -i "BLOGSKIP" | grep "tun" | grep "tap")
		[ -z "${skip_rule}" ] && {
			fw_s_add 4 m FORWARD BLOGSKIP 1 { "-i $server_dev" }
			fw_s_add 4 m FORWARD BLOGSKIP 1 { "-o $server_dev" }
		}
    }

    local mss_rule=$(fw list 4 m FORWARD | grep -i "zone_wan_ovpns_MSSFIX")
    [ -z "${mss_rule}" ] && {
        fw add 4 m zone_wan_ovpns_MSSFIX
        fw add 4 m FORWARD zone_wan_ovpns_MSSFIX 1
    }
    fw_set_ovpnmss "tunS" "zone_wan_ovpns_MSSFIX"
}
...
fw_openvpn_block(){
	...
	fw_s_del 4 f forwarding_rule_vpn ACCEPT { "-i $server_dev -o $server_dev" }
    else
    ...
    if [ "$enabled" = "off" -o "$vpntype" != "openvpn" ]; then
        [ -e /proc/fcache/ ] && {
            local skip_rule=$(fw list 4 m FORWARD | grep -i "BLOGSKIP" | grep "tun" | grep "tap")
            [ -n "${skip_rule}" ] && {
                fw_s_del 4 m FORWARD BLOGSKIP { "-i $server_dev" }
                fw_s_del 4 m FORWARD BLOGSKIP { "-o $server_dev" }
            }
        }
    fi
    ...
    local mss_rule=$(fw list 4 m FORWARD | grep -i "zone_wan_ovpns_MSSFIX")
    [ -n "${mss_rule}" ] && {
        fw del 4 m FORWARD zone_wan_ovpns_MSSFIX
        fw del 4 m zone_wan_ovpns_MSSFIX
    }
}
