...
iptv_wan_set_ifname()
{
	...
	local wan_profile_ifname=$(uci get profile.@wan[0].wan_ifname -c "/etc/profile.d" -q)
	local model=$(uci get profile.@global[0].model -c "/etc/profile.d" -q)

	if [ "$model" = "QCA_IPQ50XX" ]; then
		if [ "$iptv_mode" = "Bridge" ]; then
			lua /lib/iptv/iptv_config.lua set network device name $wan_profile_ifname
			uci_toggle_state iptv core last_wan_vif $wan_profile_ifname
		else
			lua /lib/iptv/iptv_config.lua set network device name "$1"
			uci_toggle_state iptv core last_wan_vif $1
		fi
	fi
