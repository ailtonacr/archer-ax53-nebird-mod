...
iptv_reset_wan()
{
	iptv_sync_wan_device "$WAN_PHY_IF"
	iptv_wan_set_ifname "$WAN_PHY_IF"
}

#add by wanghao
iptv_sync_wan_device()
{
	local old_name=$(uci get network.dev_wan.name -q)
	local new_name="$1"
	local default_name
	
	if [ -z "$old_name" ]; then
		default_name=$(uci get profile.@wan[0].wan_ifname -c "/etc/profile.d" -q)
		[ -n "$default_name" ] && {
			for i in $(seq 0 1)
			do
				old_name=$(uci get network.@device[$i].name -q)
				if [ "$old_name" == "$default_name" ]; then
					uci rename network.@device[$i]=dev_wan
					if [ "$old_name" != "$new_name" ]; then
						uci set network.@device[$i].name="$new_name"
					fi
					uci commit network
				fi
			done
		...
		if [ "$old_name" != "$new_name" ]; then
			uci set network.dev_wan.name="$new_name"
			uci commit network
		fi
	fi
}
...
internet_vlan_do_ex()
{
	...
	[ $internet_vid -gt 0 ] && {
		...
		iptv_sync_wan_device "$wan_env_vif"
...
mcast_vlan_do_ex()
{
	...
	[ $iptv_vid -gt 0 -a $mcast_vid -gt 0 -a -n "$ports" ] && {
		...
		for if in $mciptv_ifs; do
			append lan_vid_map ${if##*.}
		done
...
iptv_load()
{
	...
	[ "$wtype" != "none" ] && {
		...
		uci_set_state iptv core iptv_tag "$iptv_tag"
		uci_set_state iptv core ipphone_tag "$ipphone_tag"
...
destroy_current_wan_vif()
{
	vif_to_del=$(uci_get_state iptv core last_wan_vif)
	vconfig rem $vif_to_del
}
...
iptv_start()
{
	...
	[ "$iptv_enable" = "on" -o "$iptv_igmp_snooping_enable" = "on" ] && {	
		local wan_save_vif=$(uci_get_state iptv core last_wan_vif)
		uci_revert_state iptv
		uci_set_state iptv core "" state
		uci_set_state iptv core enable 1
		uci_set_state iptv core last_wan_vif $wan_save_vif
	...
	[ "$iptv_enable" != "on" -a "$iptv_igmp_snooping_enable" != "on" ] && {
		if [ -f "/tmp/dut_bootdone" ]; then 
	...
	[ "$restarted" = "1" ] && {
		echo reset > /proc/driver/rtl8367s/lut
		switch_link_down
		/etc/init.d/network restart
		# link up switch after switch vlan configure (switch/65-iptv) to fix bug-938339
		# switch_link_up
	}
	#iptv_stop
}
