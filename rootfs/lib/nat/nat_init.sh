...
nat_env_parse_done() {
    ...
    # if the internet connection type is dslite, determine the connection status of the WAN port based on the status of dslt-wan
    local wanproto=$(uci get network.wan.wan_type)
    [ "$wanproto" = "dslite" ] && {
        local device=
        local conn_state=
        network_get_device device "wan"
        [ -n "$device" -a "$device" == "dslt-wan" ] && network_get_link conn_state "$device"
        [ -n "$conn_state" -a "x$conn_state" == "x1" ] && return 0
	}
...
nat_config_rule_operation() {
    ...
    then
        ...
        nat_chain_flush nat ${nat_rule_chains}_vsgaming
