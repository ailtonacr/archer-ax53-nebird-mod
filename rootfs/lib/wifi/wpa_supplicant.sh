...
wpa_supplicant_setup_vif() {
	...
	local MapBSSType=`uci get wireless.$vif.MapBSSType`
	local map=`uci get wireless.$vif.map`
	...
	local wps_pbc=`uci get wireless.$vif.wps_pbc`

	config_get config_methods "$vif" wps_config
	[ "$wps_pbc" = "on" ] && append config_methods "push_button display"
    ...
    config_get model_number wps model_number 1
    config_get wps_device_name wps wps_device_name "AX3000 4-Stream Wi-Fi 6 Router"
    config_get wps_manufacturer wps wps_manufacturer "TP-Link"
    config_get model_name wps model_name "Archer AX55"
    config_get serial_number wps serial_number "Archer AX55"
...
EOF
	...
	echo "proto=$proto,key_mgmt=$key_mgmt" > /dev/console
	[ -z "$proto" -a "$key_mgmt" != "NONE" ] || {\
		...
		pid=`cat /var/run/wpa_supplicant-$ifname.pid`
		if [ -n "$pid" ];then
			kill -9 $pid
		fi
		wpa_supplicant -b $bridge -B -P /var/run/wpa_supplicant-$ifname.pid -D $driver -i $ifname -c /var/run/wpa_supplicant-$ifname.conf
		touch /var/run/wpa_supplicant-$ifname.lock
	}

	local mesh_enable=`uci get meshd.meshd.enable`
	if [ "$mesh_enable" = "on" ]; then
		wpa_cli -i $ifname -p /var/run/wpa_supplicant-$ifname tp_mesh_enable 1
		wpa_cli -i $ifname -p /var/run/wpa_supplicant-$ifname disconnect
		wpa_cli -i $ifname -p /var/run/wpa_supplicant-$ifname scan_interval 30
	fi
