...
update_ini_for_mon_buf_ring_chainmask_2()
{
...
unload_qcawificfg80211() {
	local recover=$1
	config_load wireless
	config_foreach disable_qcawificfg80211 wifi-device
...
post_qcawificfg80211() {
	...
	case "${action}" in
		enable)
			...
			config_load wireless
			if [ -f  "/lib/wifi/iface-mgr.sh" ]; then
	...
	*)
	...
	if [ -e /sys/firmware/devicetree/base/MP_256 ]; then
		...
		*)
		...
		[  ! -e  /etc/modules.d/53-diag-char ] &&  {
			echo diagchar > /etc/modules.d/53-diag-char
			insmod_cmd diagchar
		}
	fi
