...
setup_duplex() {
	local port=$(uci get portspeed.Cap1000.port)
	local speed=$(uci get portspeed.Cap1000.current)
	local autoneg
	local duplex
	...
	#We need to configure LAN and WAN at startup(both ap and router)
	set_port_duplex $port $speed $duplex $autoneg
	set_port_duplex ${lan_ports:0:1} "1000" "full" "on"
