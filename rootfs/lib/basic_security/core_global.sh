...
fw_load_global() {
	...
	local icmp="-p icmp -m icmp --icmp-type 8"
	local icmpv6="-p ipv6-icmp -m icmp6 --icmpv6-type 128"
	...
	network_get_ipaddr6 br_lanv6 lanv6
	...
	fw_spi_update "$global_spi_enable"

	#we should let icmpv6/dhcpv6 packet passthrough
	...
	case $global_wan_ping in
		on )
			...
			fw s_del 6 f zone_wan DROP { "$icmpv6" }
		;;
		off )
			fw s_del 4 f zone_wan ACCEPT { "$icmp" }
			...
			fw s_add 6 f zone_wan DROP 1 { "$icmpv6" }
			network_get_ipaddr6 wanv6_ip wanv6
			[ -n "$wanv6_ip" -a "${wanv6_ip:0:6}" != "fe80::" ] && {
				conntrack -D -f ipv6 -d "$wanv6_ip" -p icmpv6 --icmpv6-type 128 --icmpv6-code 0 2>/dev/null
			}
			[ -n "$br_lanv6" -a "${br_lanv6:0:6}" != "fe80::" ] && {
				conntrack -D -f ipv6 -d "$br_lanv6" -p icmpv6 --icmpv6-type 128 --icmpv6-code 0 2>/dev/null
			}
		;;
	esac
