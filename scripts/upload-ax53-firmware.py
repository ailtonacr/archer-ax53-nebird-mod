...
Default flow:
  ...
  * validate the firmware image and print SHA256
  * log in and navigate to Firmware Upgrade
  * attach the image
  * click the first Upgrade/Atualizar button to reach the stock confirmation step
  * STOP at the confirmation step and ask in the terminal
  * only click the final confirmation after the user types CONFIRMAR
  * keep the browser alive while upload/flash is in flight and monitor reboot
...
def validate_headed_environment(headed: bool) -> None:
    ...
    if os.environ.get("DISPLAY") or os.environ.get("WAYLAND_DISPLAY"):
        return
    die(
        "--headed foi solicitado, mas esta shell não possui DISPLAY/WAYLAND_DISPLAY.\n"
        "Rode sem --headed para usar Chromium headless, ou execute a partir de uma "
        "sessão gráfica/Xvfb."
    )
...
def login_if_needed(page, password: str, username: str | None) -> None:
    ...
    if visible_in_frames(page, 'input[type="password"]') is not None:
        body = ""
        try:
...
def navigate_to_firmware(page) -> None:
    ...
    if click_named(page, FIRMWARE_RE, timeout_ms=10000) is None:
        die(
            "não encontrei Firmware Upgrade/Atualização de Firmware. "
            "A UI pode ter mudado; consulte o screenshot de erro."
        )
    ...
    for frame in all_frames(page):
        try:
            ...
            for d_idx in range(dialogs.count()):
                ...
                if not dialog.is_visible():
                    continue
                button = first_visible(dialog.get_by_role("button", name=CANCEL_RE))
...
def ask_flash_confirmation(firmware: FirmwareInfo, router_url: str) -> bool:
    ...
    print("\nO firmware já foi selecionado e a UI está no passo final de confirmação.")
...
def run_browser(args, firmware: FirmwareInfo, password: str) -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if reboot_started:
                print(
                    "WARNING: a Web UI caiu antes de aparecer um passo de confirmação. "
                    "O firmware desta UI pode iniciar o flash no primeiro clique."
                )
                if not args.no_wait_reboot:
                    wait_for_reboot(host, port, args.reboot_timeout, already_down=True)
                return

            if confirm_button is None:
                shot = screenshot(
                    page,
                    f"failed-confirm-build-{firmware.build}-{int(time.time())}.png",
                )
                extra = f" Screenshot: {shot}" if shot else ""
                die(
                    "após clicar Upgrade, nenhum passo final de confirmação foi "
                    f"detectado em 90s. ABORT sem enviar a confirmação.{extra}"
                )
            ...
            if not ask_flash_confirmation(firmware, args.router_url):
                print("[confirm] flash CANCELADO pelo usuário; confirmação não enviada.")
            ...
            if confirm_button is None:
                die(
                    "a confirmação final desapareceu antes do clique. "
                    "ABORT sem tentar um clique genérico."
                )
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "Por padrão usa o último build concluído (BUILD - 1), chega até "
            "a confirmação final e pede autorização no terminal."
        )
    )
    parser.add_argument(
        "--router-url",
    ...
    parser.add_argument(
        "--headed",
        action="store_true",
        help="abre Chromium visível; requer DISPLAY/WAYLAND_DISPLAY",
    )
    parser.add_argument(
        "--ui-timeout",
        type=int,
    ...
    parser.add_argument(
        "--no-wait-reboot",
        action="store_true",
        help="não monitora a queda/retorno da UI depois da confirmação final",
    )
