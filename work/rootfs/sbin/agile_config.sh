...
global_rule_apply()
{
		...
		elif [ "A$(grep '?1ValueReserve1?' $1 |grep psk_key)" != "A" ];then
			sed -i "/^<psk_key>/s/?1ValueReserve1?/$pin/g" $1
	 	else
...
wifi_intf_name_get()
{
	...
	if [ $type = "qcawificfg80211" ]; then
		if [ $band = "2g" ]; then
			wifi_intf_24g=$(uci get profile.@wireless[0].wireless_ifname_2g -c /etc/profile.d)
			wifi_intf_24g_gst=$(uci get profile.@wireless[0].wireless_guest_ifname_2g -c /etc/profile.d)
			if [ -z $wifi_intf_24g ]; then
				wifi_intf_24g="ath0"
			fi
			if [ -z $wifi_intf_24g_gst ]; then
				wifi_intf_24g_gst="ath01"
			fi
			#Error handling in the private_ssid_apply()
			config_get guest $wifi_intf_24g_gst guest
			if [ -z $guest ]; then
			...
			wifi_intf_5g=$(uci get profile.@wireless[0].wireless_ifname_5g -c /etc/profile.d)
			wifi_intf_5g_gst=$(uci get profile.@wireless[0].wireless_guest_ifname_5g -c /etc/profile.d)
			if [ -z $wifi_intf_5g ]; then
				wifi_intf_5g="ath1"
			fi
			if [ -z $wifi_intf_5g_gst ]; then
				wifi_intf_5g_gst="ath11"
			fi
			#Error handling in the private_ssid_apply()
			config_get guest $wifi_intf_5g_gst guest
			if [ -z $guest ]; then
...
private_ssid_apply()
{
		...
		wifi_intf_24g=$(uci get profile.@wireless[0].wireless_ifname_2g -c "/etc/profile.d" -q)
		wifi_intf_24g_gst=$(uci get profile.@wireless[0].wireless_guest_ifname_2g -c "/etc/profile.d" -q)
		wifi_intf_5g=$(uci get profile.@wireless[0].wireless_ifname_5g -c "/etc/profile.d" -q)
		wifi_intf_5g_gst=$(uci get profile.@wireless[0].wireless_guest_ifname_5g -c "/etc/profile.d" -q)
...
{
 		grep -w "password\|SSID_2G_0\|SSID_2G_1\|SSID_5G_0\|SSID_5G_1\|wirelesskey\|PPPOE4_username\|PPPOE4_password\|static_IP4\|static_Mask4\|static_GW4\|static_DNS4" $2
