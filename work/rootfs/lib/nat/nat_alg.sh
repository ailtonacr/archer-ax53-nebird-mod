...
nat_config_load_alg() {
    ...
    nat__do_vpn_passthrough() {
        ...
        [ "$enable" == "off" ] && {
            ...
            [ -f /tmp/natalg_done ] || nat_syslog 22 "$vpn_upper"
            return
        }
        ...
        [ -f /tmp/natalg_done ] || nat_syslog 21 "$vpn_upper"
    }
    ...
    nat__do_alg() {
        ...
        [ "$enable" == "on" ] && {
            ...
            [ -f /tmp/natalg_done ] || nat_syslog 21 "$upper_alg"
            return     
        }
        ...
        [ -f /tmp/natalg_done ] || nat_syslog 22 "$upper_alg"
    }
    local wanproto=$(uci get network.wan.wan_type)
		[ "$wanproto" = "dslite" -o "$wanproto" = "v6plus" ] && {
			local alg_ftp_support=$(uci get profile.@"$wanproto"[0].alg_ftp_support -q -c /etc/profile.d/) || local alg_ftp_support="no"
			local alg_tftp_supprot=$(uci get profile.@"$wanproto"[0].alg_tftp_supprot -q -c /etc/profile.d/) || local alg_tftp_supprot="no"
			local alg_h323_support=$(uci get profile.@"$wanproto"[0].alg_h323_support -q -c /etc/profile.d/) || local alg_h323_support="no"
			local alg_rtsp_support=$(uci get profile.@"$wanproto"[0].alg_rtsp_support -q -c /etc/profile.d/) || local alg_rtsp_support="no"
			local alg_sip_support=$(uci get profile.@"$wanproto"[0].alg_sip_support -q -c /etc/profile.d/) || local alg_sip_support="no"
			local alg_pptp_support=$(uci get profile.@"$wanproto"[0].alg_pptp_support -q -c /etc/profile.d/) || local alg_pptp_support="no"
			local alg_l2tp_support=$(uci get profile.@"$wanproto"[0].alg_l2tp_support -q -c /etc/profile.d/) || local alg_l2tp_support="no"
			local alg_ipsec_support=$(uci get profile.@"$wanproto"[0].alg_ipsec_support -q -c /etc/profile.d/) || local alg_ipsec_support="no"
			[ -n "$nat_alg_ftp" ] && [ "$alg_ftp_support" != "yes" ] && nat_alg_ftp="off"
			[ -n "$nat_alg_tftp" ] && [ "$alg_tftp_supprot" != "yes" ] && nat_alg_tftp="off"
			[ -n "$nat_alg_h323" ] && [ "$alg_h323_support" != "yes" ] && nat_alg_h323="off"
			[ -n "$nat_alg_rtsp" ] && [ "$alg_rtsp_support" != "yes" ] && nat_alg_rtsp="off"
			[ -n "$nat_alg_sip" ] && [ "$alg_sip_support" != "yes" ] && nat_alg_sip="off"
			[ -n "$nat_alg_pptp" ] && [ "$alg_pptp_support" != "yes" ] && nat_alg_pptp="off"  
			[ -n "$nat_alg_l2tp" ] && [ "$alg_l2tp_support" != "yes" ] && nat_alg_l2tp="off"
			[ -n "$nat_alg_ipsec" ] && [ "$alg_ipsec_support" != "yes" ] && nat_alg_ipsec="off"
		}
