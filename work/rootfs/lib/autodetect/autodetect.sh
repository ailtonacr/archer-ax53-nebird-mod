...
TRANSIX_URL="gw.transix.jp"
XPASS_URL="dgw.xpass.jp"
V6CONNECT_URL="dslite.v6connect.net"
...
BIGLOBE_IP_PRE_32="2404:7a80 2404:7a81 2404:7a82 2404:7a83 2404:7a84 2404:7a85 2404:7a86 2404:7a87" #2404:7a80::/30 2404:7a84::/30
...
wan_get_proto() {
	...
	if [ "$proto" == "v6plus" ]; then
		...
		[ $carrier -eq 2 ] && proto="ipv6_option"
	fi

	if [ "$proto" == "dslite" ]; then
		config_load /etc/config/protocol
		config_get mode dslite mode
		proto=${mode}
	fi
...
check_prefix() {
	local prefix=$1
	local is_ipv6_option="no"

	#No conversion of number systems, just check if the first 32 bits of the address match
	for i in $BIGLOBE_IP_PRE_32; do
		if [ "$i" == "$prefix"  ]; then
			is_ipv6_option="yes"
			break
		fi
...
check_v6ip_in_BIGLOBE() {
	...
	do
		...
		element=${line:0:32}
		prefix=${element:0:4}:${element:4:4}
	done </proc/net/if_inet6

	#Check if ip matches prefix of BIGLOBE(2404:7a80::/30 2404:7a84::/30)
	is_BIGLOBE=$(check_prefix $prefix)
...
check_v6plus() {
	...
	[ -n "$dns1" ] || [ -n "$dns2" ] && {
		...
		[ "$urls_status_code" = "200" ] && {
			...
			#check if is BIGLOBE by ipv6_addr
			is_ipv6_option="no"
			is_ipv6_option=$(check_v6ip_in_BIGLOBE)

			#default v6plus
			if [ "$is_ipv6_option" == "yes" ]; then
				record_clean_and_exit "ipv6_option"
			else
				record_clean_and_exit "v6plus"
			fi
		}
...
get_dslite_mode(){
	local domain=$1
	[ $domain == $TRANSIX_URL ] && echo "transix" && return
	[ $domain == $XPASS_URL ] && echo "xpass" && return
	[ $domain == $V6CONNECT_URL ] && echo "v6_connect" && return
	...
	[ -n "$dns1" ] || [ -n "$dns2" ] && {
		[ -n "$AFTR_name" ] && [ "$AFTR_name" != "$DSLITE_DOMAIN" ] && {
			...
			dnslookup -6 -t $DNS_V6_TIMEOUT "$AFTR_name" $dns2 && \
				record_clean_and_exit "$(get_dslite_mode $AFTR_name)"
		}
		...
		if [ -n "$DSLITE_AFTR_LIST_PRIVATE" -o -n "$DSLITE_AFTR_LIST_PUBLIC" ]; then
			for domain in $DSLITE_AFTR_LIST_PRIVATE; do
				...
				dnslookup -6 -t $DNS_V6_TIMEOUT "$domain" $dns2 && \
					record_clean_and_exit "$(get_dslite_mode $domain)"
			done
			for domain in $DSLITE_AFTR_LIST_PUBLIC; do
				...
				for addr in $addr_ipv6; do
					ping -6 $addr -c 1 -W 3 && record_clean_and_exit "$(get_dslite_mode $domain)"
				done
			done
		else
			dnslookup -6 -t $DNS_V6_TIMEOUT "$DSLITE_DOMAIN" $dns1 || \
			dnslookup -6 -t $DNS_V6_TIMEOUT "$DSLITE_DOMAIN" $dns2 && \
				record_clean_and_exit "$(get_dslite_mode $DSLITE_DOMAIN)"
		fi
	}
}
...
autodial_flag=`uci get network.wan.autodial_flag -q`
online-test "$CHECK_URL" "$DNS_FIRST_TIMEOUT" && {
    if [ "$autodial_flag" = 1 ]; then
        local http_code=$(curl -o /dev/null -skL -4 -m 10 -w %{http_code} tp-link.com)
        [ "$http_code" = "200" -o "$http_code" = "301" ] && record_clean_and_exit "$(wan_get_proto)"
    else
