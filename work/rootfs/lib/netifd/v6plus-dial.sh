	...
	case $v6plus_state in 
		...
		"4"| 4 )
			...
			while ! network_get_ipaddr6 ip6addr "$tunlink"; do
                ...
                let "count=count - 1"
                if [ $count -eq 0 ];then
                    echo "IPV6_ADDR_TIMEOUT" > /tmp/connecterror
                    count=15
                fi
                sleep 2
			done
		...
		"8"| 8 )
			...
			echo "RULES_MAP_FAILED: Timeout Random(10min-30min) " > /dev/console
			 echo "RULES_MAP_ERROR" > /tmp/connecterror
			;;
		"9"| 9 )
			#GET_RULES_40XERROR
			proto_init_update "$link"
		...
		"13"| 13 )
			...
			uci set dsliteV6plus.wan.device6=$l3_device
			uci commit	
			...
			#echo "[v6plus-dial.sh]:disable v6plus-wan tso" > /dev/console
