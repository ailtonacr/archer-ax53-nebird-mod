...
local function set_ipv6_option()
	local http_form = {}
	http_form.form = "wan_ipv4_ipv6_option"
	http_form.operation = "write"
	http_form.conntype = "ipv6_option"
...
local function auto_dial()
	...
	if autodial_flag == "1" then
		...
		if conntype == "v6plus" then
			...
			unlock()
			return ret
		elseif conntype == "dslite" or conntype == "transix" or conntype == "xpass" or conntype == "v6_connect" then
			local mode = conntype
			conntype = "dslite"
			uci_r:set("wportal", "defcfg", "defcfg", "no")
			uci_r:commit("wportal")
			...
			unlock()
			return ret
		elseif conntype == "ipv6_option" then
			uci_r:set("wportal", "defcfg", "defcfg", "no")
			uci_r:commit("wportal")
			ret = set_ipv6_option()
			if ret == true then
