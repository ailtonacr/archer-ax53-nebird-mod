...
hostapd_set_bss_options() {
	...
	# Wireless configuration has been loaded in /sbin/wifi, and must be fixed.
	...
	[ -z "$MapBSSType" ] && MapBSSType=`uci get onemesh.$vif.MapBSSType`
	[ -z "$map" ] && map=`uci get onemesh.$vif.map`
	
	[ -n "$map" -a "$map" != "0" ] && append "$var" "multi_ap_profile=$map" "$N"
...
hostapd_setup_vif() {
	...
	config_get edge_channel_deprioritize "$device" edge_channel_deprioritize 0
	config_get acs_freq_list "$vif" acs_freq_list
	config_get band "$device" qca_band 0
	config_get acs_6g_only_psc "$vif" acs_6g_only_psc 0

	# For repeater, 5G channel is set to 40
	local role=`uci get sysmode.sysmode.mode`
	[ "$role" = "repeater" -a "$device" = "wifi1" ] && channel=40
...
# FOR IPF, config_load will cause wifi-device to be cleaned, so that setup_wps_rf_band_info is not called
#	config_load fst && {
