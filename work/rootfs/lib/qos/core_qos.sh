...
# for rtl hw_qos
rtl_hqos_support=$(uci get profile.@qos[0].rtl_hqos_support -c "/etc/profile.d" -q)
...
fw_rule_load() {
    ...
    if [[ "${bcm_hqos_support}" == "yes" ]]; then
		...
		if [[ "${nqos_support}" == "yes" ]]; then
			if [ "$wireless_qos_support" = "yes" ]; then
				local lan_nqos_target="CLASSIFYMASK --set-class 1101:0010/0xfffffff0"
				local wan_nqos_target="CLASSIFYMASK --set-class 2101:0020/0xfffffff0"
			else
    ...
    if [[ "${bcm_hqos_support}" == "yes" ]]; then
		...
		if [[ "${nqos_support}" == "yes" ]]; then
			if [ "$wireless_qos_support" = "yes" ]; then
				lan_nqos_target="CLASSIFYMASK --set-class 1103:0030/0xfffffff0"
				wan_nqos_target="CLASSIFYMASK --set-class 2103:0040/0xfffffff0"
			else
...
fw_load_global() {
    ...
    if [[ x"$(uci_get_state qos_v2 core loaded)" != x1 ]]; then
        if [[ "$global_enable" == "on" ]]; then
            ...
            # for rtl hw_qos, enable hw_qos
            if [[ "${rtl_hqos_support}" == "yes" ]]; then
                echo 1 > /proc/qos
                echo "restore eth1 dscp remark" > /proc/rtl_hw_qos_config
                echo "restore eth1 vlan remark" > /proc/rtl_hw_qos_config
                echo "restore br-lan dscp remark" > /proc/rtl_hw_qos_config
                echo "restore br-lan vlan remark" > /proc/rtl_hw_qos_config
                echo "disable dscp remark" > /proc/rtl_hw_qos_config
                echo "disable vlan remark" > /proc/rtl_hw_qos_config
                echo nat 1 > /proc/rtl_83xxQos_config
            fi
            ...
            if [[ "${rtl_hqos_support}" == "yes" ]]; then
                ...
                echo "disable dscp remark" > /proc/rtl_hw_qos_config
                echo "disable vlan remark" > /proc/rtl_hw_qos_config
                echo nat 0 > /proc/rtl_83xxQos_config
            fi
...
{
	local ifname=$1
	local port=$(echo $ifname | egrep -o "[0-9]*")
	echo $port
}
...
fw_tc_start() {
    ...
    for i in $ifaces; do
        ...
        #for bcm, HQoS support
        if [[ "${bcm_hqos_support}" == "yes" ]]; then
            local bcm_hqos_dsq_nodir=$(uci get profile.@qos[0].bcm_hqos_dsq_nodir -c "/etc/profile.d" -q)
            local qsize=682
            ...
            if [ x"${bcm_hqos_dsq_nodir}" != x"yes" ]; then
                ...
                local sub_base=16
                local port_index=$(tm_get_port_index ${wan_ifname})
                local sub0_index=$((${sub_base}))
                local sub1_index=$((${sub_base} + 1))
                local sub2_index=$((${sub_base} + 2))

                uplink_hqos=$((${uplink_hqos} * 1000))
                sp_max_speed=$((${sp_max_speed} * 1000))

                tmctl porttmuninit --devtype 0 --if ${wan_ifname};
                bs /b/n egress_tm/index=${port_index},level=egress_tm,mode=sp port/name=${wan_ifname};

                bs /b/n egress_tm/index=${sub0_index},level=queue,mode=wrr,num_queues=2,weight=1 egress_tm/index=${port_index};
                bs /b/c egress_tm/index=${sub0_index} queue_cfg[0]={queue_id=0,drop_threshold=${threshold},weight=1,rl={af=0,be=0,burst=0}};
                bs /b/c egress_tm/index=${sub0_index} queue_cfg[1]={queue_id=1,drop_threshold=${threshold},weight=1,rl={af=0,be=0,burst=0}};
                bs /b/c egress_tm/index=${port_index} subsidiary[0]={egress_tm/index=${sub0_index}};

                bs /b/n egress_tm/index=${sub1_index},level=queue,mode=wrr,rl={af=${uplink_hqos},be=0,burst=0},rl_rate_mode=dual_rate,num_queues=2,weight=1 egress_tm/index=${port_index};
                bs /b/c egress_tm/index=${sub1_index} queue_cfg[0]={queue_id=2,drop_threshold=${threshold},weight=${weight_low},rl={af=0,be=0,burst=0}};
                bs /b/c egress_tm/index=${sub1_index} queue_cfg[1]={queue_id=3,drop_threshold=${threshold},weight=${weight_high},rl={af=0,be=0,burst=0}};
                bs /b/c egress_tm/index=${port_index} subsidiary[1]={egress_tm/index=${sub1_index}};

                bs /b/n egress_tm/index=${sub2_index},level=queue,mode=sp,rl={af=${sp_max_speed},be=0,burst=0},rl_rate_mode=dual_rate,num_queues=4 egress_tm/index=${port_index};
                bs /b/c egress_tm/index=${sub2_index} queue_cfg[0]={queue_id=4,drop_threshold=${threshold}};
                bs /b/c egress_tm/index=${sub2_index} queue_cfg[1]={queue_id=5,drop_threshold=${threshold}};
                bs /b/c egress_tm/index=${sub2_index} queue_cfg[2]={queue_id=6,drop_threshold=${threshold}};
                bs /b/c egress_tm/index=${sub2_index} queue_cfg[3]={queue_id=7,drop_threshold=${threshold}};
                bs /b/c egress_tm/index=${port_index} subsidiary[2]={egress_tm/index=${sub2_index}};
            fi
...
#			tcq_d qdisc add dev $wan_ifname parent 1:0030 handle 1103:0030 nsspfifo limit $limit
			qos_tc_add_nqos_ln_qdisc $wan_ifname 1:0010 1101:0010
			qos_tc_add_nqos_ln_qdisc $wan_ifname 1:0030 1103:0030

        elif [[ "${rtl_hqos_support}" == "yes" ]]; then
            # uplink high 1:2
            tc_d class add dev $wan_ifname parent 1:1 classid 1:2 htb rate "$up_high" ceil 1024Mbit $u_hi_burst prio 2 quantum 1500
            tc_d qdisc add dev $wan_ifname parent 1:2 handle 12: sfq perturb 10
            # uplink low 1:5
            tc_d class add dev $wan_ifname parent 1:1 classid 1:5 htb rate "$up_low" ceil "$uplink" $u_lo_burst prio 2 quantum 1500
            tc_d qdisc add dev $wan_ifname parent 1:5 handle 15: sfq perturb 10
            # uplink filter
            tc_d filter add dev $wan_ifname parent 1:0 protocol all prio 100 handle 0x0010/0xfff0 fw classid 1:2
            tc_d filter add dev $wan_ifname parent 1:0 protocol all prio 100 handle 0x0030/0xfff0 fw classid 1:5
...
#		tcq_d qdisc add dev $lanDev parent 2:0040 handle 2103:0040 nsspfifo limit $limit
		qos_tc_add_nqos_ln_qdisc $lanDev 2:0020 2101:0020
		qos_tc_add_nqos_ln_qdisc $lanDev 2:0040 2103:0040
	elif [[ "${rtl_hqos_support}" == "yes" ]]; then
		# downlink high 2:2
		tc_d class add dev $lanDev parent 2:1 classid 2:2 htb rate "$down_high" ceil 1024Mbit $u_hi_burst prio 2 quantum 1500
		tc_d qdisc add dev $lanDev parent 2:2 handle 22: sfq perturb 10
		# downlink low 2:5
		tc_d class add dev $lanDev parent 2:1 classid 2:5 htb rate "$down_low" ceil "$downlink" $u_lo_burst prio 2 quantum 1500
		tc_d qdisc add dev $lanDev parent 2:5 handle 25: sfq perturb 10
		# downlink filter
		tc_d filter add dev $lanDev parent 2:0 protocol all prio 100 handle 0x0020/0xfff0 fw classid 2:2
		tc_d filter add dev $lanDev parent 2:0 protocol all prio 100 handle 0x0040/0xfff0 fw classid 2:5
...
#			tcq_d qdisc del dev $wan_ifname parent 1:0030 handle 1103:0030 nsspfifo limit $limit
			qos_tc_del_nqos_ln_qdisc $wan_ifname 1:0010 1101:0010 
			qos_tc_del_nqos_ln_qdisc $wan_ifname 1:0030 1103:0030
            ...
            tc_d filter del dev $wan_ifname parent 1:0 protocol all prio 100 handle 0x0010/0xfff0 fw classid 1:2
            tc_d filter del dev $wan_ifname parent 1:0 protocol all prio 100 handle 0x0030/0xfff0 fw classid 1:5
            tc_d class del dev $wan_ifname parent 1:1 classid 1:2
            tc_d class del dev $wan_ifname parent 1:1 classid 1:5
		else
...
#		tcq_d qdisc del dev $lanDev parent 2:0040 handle 2103:0040 nsspfifo limit $limit
		qos_tc_del_nqos_ln_qdisc $lanDev 2:0020 2101:0020
		qos_tc_del_nqos_ln_qdisc $lanDev 2:0040 2103:0040
        ...
        tc_d filter del dev $lanDev parent 2:0 protocol all prio 100 handle 0x0020/0xfff0 fw classid 2:2
        tc_d filter del dev $lanDev parent 2:0 protocol all prio 100 handle 0x0040/0xfff0 fw classid 2:5
        tc_d class del dev $lanDev parent 2:1 classid 2:2
        tc_d class del dev $lanDev parent 2:1 classid 2:5
    else
...
fw_add_tc_root() {
    ...
    if [ -z "${wan_rule_root}" ] ; then
        for i in $ifaces; do
			...
			# for rtl hw_qos
			if [[ "${rtl_hqos_support}" == "yes" ]]; then
				tc_d qdisc add dev $wan_ifname root handle 1:0 htb default 2 r2q 64
				tc_d class add dev $wan_ifname parent 1:0 classid 1:1 htb rate 1024Mbit ceil 1024Mbit $up_burst quantum 1500
			elif [ "$nqos_support" != "yes" ]; then
...
#				tcq_d qdisc add dev $wan_ifname parent 1:11 handle 1100: nsspfifo limit $limit set_default
				qos_tc_add_nqos_ln_qdisc $wan_ifname 1:11 1100: "set_default"
			fi
        done
    else
    	for i in $ifaces; do
			...
			# for rtl hw_qos
			if [[ "${rtl_hqos_support}" == "yes" ]]; then
				tc_d class replace dev $wan_ifname parent 1:0 classid 1:1 htb rate 1024Mbit ceil 1024Mbit $up_burst quantum 1500
			elif [ "$nqos_support" != "yes" ]; then
    ...
    if [ -z "${lan_rule_root}" ] ; then
		...
		# for rtl hw_qos
		if [[ "${rtl_hqos_support}" == "yes" ]]; then
			tc_d qdisc add dev $lanDev root handle 2:0 htb default 2 r2q 64
			tc_d class add dev $lanDev parent 2:0 classid 2:1 htb rate 1024Mbit ceil 1024Mbit $down_burst quantum 1500
		elif [ "$nqos_support" != "yes" ]; then
...
#			tcq_d qdisc add dev $lanDev parent 2:11 handle 2100: nsspfifo limit $limit set_default
			qos_tc_add_nqos_ln_qdisc $lanDev 2:11 2100: "set_default"
		fi
    else
		# for rtl hw_qos
		if [[ "${rtl_hqos_support}" == "yes" ]]; then
			tc_d class replace dev $lanDev parent 2:0 classid 2:1 htb rate 1024Mbit ceil 1024Mbit $down_burst quantum 1500
		elif [ "$nqos_support" != "yes" ]; then
...
add_tc_support() {
	...
	if [ "$nqos_support" = "yes" ]; then
		if [ "$wireless_qos_support" = "yes" ]; then
			insmod /lib/modules/iplatform/xt_CLASSIFYMASK.ko
		else
...
del_tc_support() {
	...
	if [ "$nqos_support" = "yes" ]; then
		if [ "$wireless_qos_support" = "yes" ]; then
			rmmod /lib/modules/iplatform/xt_CLASSIFYMASK.ko
		else
...
update_ip_use_legacy_tos() {
    local support_qos="yes"
    local support_ffs=$(uci get profile.@wireless[0].ffs -c /etc/profile.d/ -q)
    local support_gbc=$(uci get profile.@wireless[0].guestnetwork_bandwidth_ctrl_support -c /etc/profile.d/ -q)
    local support_csl=$(uci get profile.@client_speed_limit[0].support -c /etc/profile.d/ -q)
    local support_mscs=$wireless_qos_support
    local is_enable_qos="false"
    local is_enable_ffs="false"
    local is_enable_gbc="false"
    local is_enable_csl="false"
    local is_enable_mscs="false"
    ...
    # mscs
    if [ "${support_mscs}" = "yes" ]; then
        if [ "$(uci -q get wireless.wifi0.mscs_enable)" = "on" -o "$(uci -q get wireless.wifi1.mscs_enable)" = "on" ]; then
            is_enable_mscs="true"
        fi
    fi

    if [ "${is_enable_qos}" = "true" -o "${is_enable_ffs}" = "true" -o "${is_enable_gbc}" = "true" -o "${is_enable_csl}" = "true" -o "${is_enable_mscs}" = "true" ]; then
