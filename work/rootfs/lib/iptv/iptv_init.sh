...
get_iptv_cfg()
{
	...
	if [ -z "$internet_item" -a -z "$iptv_item" -a -z "$ipphone_item" -a -z "$mciptv_item" ]; then
		...
		if [ "$iptv_enable" == "on" ]; then
			...
			if [ "$model" = "QCA_IPQ95XX" ]; then
				#process tag-off situations, in which we should not pick a vid for iptv/internet
				if [ "$iptv_mode" = "Custom" ]; then
					#there can only be one tag off for internet+iptv+voip. Assert:when tag is off, the related vid must be zero.
					if [ "$internet_tag" = "off" ]; then
						#start from 4000 to select internet_vid,automatically avoiding all lan vids:see get_lan_vid().
						internet_vid=0
					elif [ "$iptv_tag" = "off" ]; then
						#start from 3000 to select iptv_vid,automatically avoiding all lan vids:see get_lan_vid().
						iptv_vid=0
					elif [ "$ipphone_tag" = "off" ]; then
						#start from 2000 to select iptv_vid,automatically avoiding all lan vids:see get_lan_vid().
						ipphone_vid=0
					fi

					echo "!!!iptv/internet tag off and Custom mode:$internet_vid+$iptv_vid+$ipphone_vid+$mciptv_vid" > /dev/console

					local _iptv_enable=$(uci get iptv.Custom.iptv_enable 2>/dev/nulll)
					local _ipphone_enable=$(uci get iptv.Custom.ipphone_enable 2>/dev/nulll)
					local port_type=$(uci get iptv.Custom.porttype 2>/dev/null)

					#Compatible with old firmware
					for i in $port_type; do
						if [ "$i" = "IPTV" ]; then
							_iptv_enable="on"
						elif [ "$i" = "IP-Phone" ]; then
							_ipphone_enable="on"
						fi
					done

					if [ "$_iptv_enable" = "off" ]; then
						#There is no IPTV interface
						iptv_vid="0"
						iptv_vprio="0"
						iptv_tag="on"
						echo "There is no iptv interface, so iptv_tag $iptv_tag iptv_vid $iptv_vid iptv_vprio $iptv_vprio" > /dev/console
					fi
					if [ "$_ipphone_enable" = "off" ]; then
						#There is no IP-phone interface
						ipphone_vid="0"
						ipphone_vprio="0"
						ipphone_tag="on"
						echo "There is no ipphone interface, so ipphone_tag $ipphone_tag ipphone_vid $ipphone_vid ipphone_vprio $ipphone_vprio" > /dev/console
					fi
				fi
			else
				#process tag-off situations, in which we should choose a random tag for iptv/internet/voip and mark switch-wan untagged.
				local _internet_vid=$(uci get iptv.Custom.internet_vid 2>/dev/nulll)
				if [ "A$_internet_vid" = "A" ];then _internet_vid="0"; fi
				local _iptv_vid=$(uci get iptv.Custom.iptv_vid 2>/dev/nulll)
				if [ "A$_iptv_vid" = "A" ];then _iptv_vid="0"; fi
				local _ipphone_vid=$(uci get iptv.Custom.ipphone_vid 2>/dev/nulll)
				if [ "A$_ipphone_vid" = "A" ];then _ipphone_vid="0"; fi
				local _mc_vid=$(uci get iptv.Custom.mciptv_vid 2>/dev/nulll)
				if [ "A$_mc_vid" = "A" ];then _mc_vid="0"; fi

				if [ "$iptv_mode" = "Custom" ]; then
					#there can only be one tag off for internet+iptv+voip. Assert:when tag is off, the related vid must be zero.
					if [ "$internet_tag" = "off" ]; then
						#start from 4000 to select internet_vid,automatically avoiding all lan vids:see get_lan_vid().
						internet_vid=$(select_vid "$_iptv_vid $_ipphone_vid $_mc_vid" 4000)
					elif [ "$iptv_tag" = "off" ]; then
						#start from 3000 to select iptv_vid,automatically avoiding all lan vids:see get_lan_vid().
						iptv_vid=$(select_vid "$_internet_vid $_ipphone_vid $_mc_vid" 3000)
					elif [ "$ipphone_tag" = "off" ]; then
						#start from 2000 to select iptv_vid,automatically avoiding all lan vids:see get_lan_vid().
						ipphone_vid=$(select_vid "$_internet_vid $_iptv_vid $_mc_vid" 2000)
					fi
...
get_iptv_lan_interface()
{
	...
	if [ "${iptv_enable}" = "on" -a -n "$lanportseq" ] ; then
		...
		[ "${mciptv_enable}" = "on" -a  "$mciptv_vid" -ne 0 ] && {
			...
			for i in $iptvPort ; do
			echo $used_vids
			vid=$(select_vid "$used_vids")
			initif="$(get_landev_by_port $i)"
			initif=${initif%%.*}
			append mciptv_ifs "${initif}.${vid}"
			echo "=======================used_vids:$used_vids select mc iptv lan vid $vid" > /dev/console
			append used_vids "$vid"
			done
		}
