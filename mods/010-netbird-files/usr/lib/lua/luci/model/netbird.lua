...
local function merged(c) local u,e=sanitize(c or {},false); if not u then return nil,e end; local cur=read_settings(); for k,v in pairs(u) do cur[k]=v end; for k,s in pairs(KEYS) do if cur[k]==nil then cur[k]=s.default end end; if cur.advertise_lan=="1" and cur.advertise_cidr=="" then return nil,"advertise_cidr required when LAN routing is enabled" end; return cur end
function preview_settings(c)return merged(c) end
...
function set_settings(c) local cur,e=merged(c); if not cur then return nil,e end; return write(cur) end
