...
#
# Reuses the LAN-only development SSH implementation proven on real hardware.
# Public-key login is already proven to enter the root shell. Password
# authentication is enabled without disabling the vendor/system-account login
# path, so the existing root behavior is preserved. Passwords are not copied
# into the firmware or /etc/shadow. The service remains LAN-only, TCP 2222,
# with no forwarding and no IPv6, using an ephemeral host key under /tmp,
# START=55.
#
# The authorized_keys file contains ONLY the public key (~/.ssh/ax53_dev.pub).
# No private key or password is ever placed in the firmware.
...
echo "### Dev SSH (dropbear :2222, key + password auth, root/system-account path preserved, LAN) ###"
echo "    rootfs: $R"
...
grep -Eq '^[[:space:]]*-L[[:space:]\\]*$' "$R/etc/init.d/devssh" || {
  echo "Error: devssh must enable SSH sessions (-L)" >&2
  exit 1
}

if grep -Eq '^[[:space:]]*-s[[:space:]\\]*$|^[[:space:]]*-g[[:space:]\\]*$' "$R/etc/init.d/devssh"; then
  echo "Error: devssh unexpectedly disables password/root-password authentication" >&2
  exit 1
fi

if grep -Eq '^[[:space:]]*-C[[:space:]\\]*$' "$R/etc/init.d/devssh"; then
  echo "Error: devssh must preserve system-account/root login; -C is not allowed" >&2
  exit 1
fi
