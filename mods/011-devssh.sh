#!/bin/bash -e
#
# 011-devssh.sh -- enable the validated development SSH (dropbear :2222) in the
# AX53 rootfs, independent of the stock /etc/init.d/dropbear.
#
# Reuses the LAN-only development SSH implementation proven on real hardware,
# now allowing both public-key authentication and the TP-Link account backend
# exposed by the vendor Dropbear (-C). Passwords are not copied into the
# firmware or /etc/shadow. The service remains LAN-only, TCP 2222, with no
# forwarding and no IPv6, using an ephemeral host key under /tmp, START=55.
#
# The authorized_keys file contains ONLY the public key (~/.ssh/ax53_dev.pub).
# No private key or password is ever placed in the firmware.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
cd "$PROJECT_ROOT"

if [ -z "${ROOTFS_DIR:-}" ]; then
  if [ -d "rootfs" ]; then ROOTFS_DIR="$PROJECT_ROOT/rootfs"
  elif [ -d "squashfs-root" ]; then ROOTFS_DIR="$PROJECT_ROOT/squashfs-root"
  else echo "Error: no rootfs dir" >&2; exit 1; fi
else
  case "$ROOTFS_DIR" in
    /*) ;;
    *) ROOTFS_DIR="$PROJECT_ROOT/$ROOTFS_DIR" ;;
  esac
fi
[ -d "$ROOTFS_DIR" ] || { echo "Error: rootfs dir does not exist: $ROOTFS_DIR" >&2; exit 1; }

FILES="$SCRIPT_DIR/011-devssh-files"
R="$ROOTFS_DIR"

echo "### Dev SSH (dropbear :2222, key + TP-Link password auth, LAN) ###"
echo "    rootfs: $R"

echo "[1/3] copying devssh init + authorized_keys ..."
(cd "$FILES" && cp -a --parents etc/init.d/devssh etc/dropbear/authorized_keys "$R/")
chmod 0755 "$R/etc/init.d/devssh"
chmod 0600 "$R/etc/dropbear/authorized_keys"

echo "[2/3] enabling service (rc.d S55devssh) ..."
mkdir -p "$R/etc/rc.d"
ln -sfn "../init.d/devssh" "$R/etc/rc.d/S55devssh"

echo "[3/3] verifying ..."
for f in etc/init.d/devssh etc/dropbear/authorized_keys etc/rc.d/S55devssh; do
  [ -e "$R/$f" ] && echo "    ok $f" || { echo "    MISSING $f" >&2; exit 1; }
done

grep -Eq '^[[:space:]]*-C[[:space:]\\]*$' "$R/etc/init.d/devssh" || {
  echo "Error: devssh must enable the TP-Link account backend (-C)" >&2
  exit 1
}

grep -Eq '^[[:space:]]*-L[[:space:]\\]*$' "$R/etc/init.d/devssh" || {
  echo "Error: devssh must enable SSH sessions (-L)" >&2
  exit 1
}

if grep -Eq '^[[:space:]]*-s[[:space:]\\]*$|^[[:space:]]*-g[[:space:]\\]*$' "$R/etc/init.d/devssh"; then
  echo "Error: devssh unexpectedly disables password/root-password authentication" >&2
  exit 1
fi

echo "### Dev SSH enabled ###"